...
export class AuthController {
    ...
    async updatePassword(
        ...
        return await this.authService.updatePassword(req['user'].uuid, updatePasswordDto);
    }

    // Invitation
    @Post('invite')
    @ApiOperation({
        summary: 'Invite to admin portal',
        description: 'This endpoint will help us to invite to admin portal.'
    })
    @ApiResponse({ status: HttpStatus.CREATED, description: UserMessages.Success.Invited })
    @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: HttpMessages.Error.InternalServerError })
    async invite(@Body() createUserDto: BaseEmailDto) {
        return await this.authService.invite(createUserDto);
    }
