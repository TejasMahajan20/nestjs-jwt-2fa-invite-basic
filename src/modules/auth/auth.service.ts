...
export class AuthService {
    ...
    async login(signInUserDto: SignInUserDto) {
        ...
        if (!await this.passwordService.comparePasswords(signInUserDto.password, userEntity.password)) {
    ...
    // Invitation
    private getLinkExpirationTime(timestamp: Date): number {
        // Calculate expiration time (24 hours later)
        return timestamp.getTime() + 24 * 60 * 60 * 1000;
    }

    private generateVerificationLink(email: string, token: string, timestamp: Date) {
        const expirationTime = this.getLinkExpirationTime(timestamp);
        const frontendOrigin = this.configService.get<string>('FRONTEND_ORIGIN');
        const verificationLink = `${frontendOrigin}/set-password?email=${email}&token=${token}&expiresAt=${expirationTime}`;
        return verificationLink;
    }

    async invite(createUserDto: BaseEmailDto) {
        // Validate user already exist or not?
        await this.userService.isUserExist(createUserDto.email);
        ...
        const { email, createdAt } = createdUser;

        const verificationLink = this.generateVerificationLink(email, password, createdAt);
    ...
    async resendVerificationLink(user: UserEntity) {
        ...
        const verificationLink = this.generateVerificationLink(user.email, password, currentDate);
